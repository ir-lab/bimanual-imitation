...
from bimanual_imitation.algorithms.core.ibc.agents import base_agent, ibc_policy, mcmc
from bimanual_imitation.algorithms.core.ibc.losses import ebm_loss, gradient_loss
...
class ImplicitBCAgent(base_agent.BehavioralCloningAgent):
    ...
    def __init__(
        ...
        assert self._fraction_dfo_samples >= 0.0
        assert self._fraction_langevin_samples >= 0.0
    ...
    def _loss(self, experience, variables_to_train=None, weights=None, training=False):
        ...
        if not self._run_full_chain_under_gradient:
            counter_example_actions, combined_true_counter_actions, chain_data = (
                self._make_counter_example_actions(
                    observations, expanded_actions, batch_size, training
                )
            )
            ...
            with tf.name_scope("loss"):
                ...
                if self._compute_mse:
                    ...
                    mse_counter_examples = common.aggregate_losses(
                        per_example_loss=mse_counter_examples
                    ).total_loss

                if self._late_fusion:
                    # Do one cheap forward pass.
                    obs_embeddings = self.cloning_network.encode(maybe_tiled_obs, training=True)
                    ...
                    network_inputs = (unused_obs, tf.stop_gradient(combined_true_counter_actions))
                    # [B * n+1]
                    predictions, _ = self.cloning_network(
                        network_inputs, training=training, observation_encoding=obs_embeddings
                    )
                ...
                if self.ebm_loss_type == "cd_kl":
                    grad_flow_network_inputs = (maybe_tiled_obs, combined_true_counter_actions)
                else:
                ...
                if self._debug_summaries:
                    common.generate_tensor_summaries(
                        "ebm_loss", per_example_loss, self.train_step_counter
                    )
    ...
    def _compute_ebm_loss(
        ...
        if self.ebm_loss_type == "info_nce":
            ...
            per_example_loss, debug_dict = ebm_loss.cd_kl(
                predictions, counter_example_actions, predictions_copy
            )
    ...
    def _make_counter_example_actions(
        ...
        random_uniform_example_actions = tensor_spec.sample_spec_nest(
            self._action_sampling_spec, outer_dims=(batch_size, self._num_counter_examples)
        )

        # If not optimizing, just return.
        if self._fraction_dfo_samples == 0.0 and self._fraction_langevin_samples == 0.0:
            ...
            random_uniform_example_actions = tf.reshape(
                random_uniform_example_actions, (batch_size * self._num_counter_examples, -1)
            )
            ...
            if self._fraction_dfo_samples > 0.0:
            ...
            if self._fraction_langevin_samples > 0.0:
            ...
            fraction_init_samples = (
                1.0 - self._fraction_dfo_samples - self._fraction_langevin_samples
            )

            # Compute indices based on fractions.
            init_num_indices = int(fraction_init_samples * self._num_counter_examples)
            dfo_num_indices = int(self._fraction_dfo_samples * self._num_counter_examples)
            langevin_num_indices = int(self._fraction_langevin_samples * self._num_counter_examples)
            residual = (
                self._num_counter_examples
                - init_num_indices
                - dfo_num_indices
