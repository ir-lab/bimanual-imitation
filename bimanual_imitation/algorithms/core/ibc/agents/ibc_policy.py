...
from bimanual_imitation.algorithms.core.ibc.agents import mcmc
...
def generate_registration_functions(policy, policy_network, strategy):
    ...
    def _create_variables(specs, training, step_type, network_state):
        return strategy.run(
            policy_network,
            args=(specs,),
            kwargs={"step_type": step_type, "network_state": network_state, "training": training},
        )
...
class IbcPolicy(tf_policy.TFPolicy):
    ...
    def __init__(
        ...
        self._action_sampling_minimum = tf.Variable(
            self._action_sampling_spec.minimum, trainable=False, name="sampling/minimum"
        )
        self._action_sampling_maximum = tf.Variable(
            self._action_sampling_spec.maximum, trainable=False, name="sampling/maximum"
        )
    ...
    def _variables(self):
        return self._actor_network.variables + [
            self._action_sampling_minimum,
            self._action_sampling_maximum,
        ]
    ...
    def _distribution(self, time_step, policy_state):
        ...
        action_samples = tensor_spec.sample_spec_nest(
            self._action_sampling_spec, outer_dims=(batch_size * self._num_action_samples,)
        )
