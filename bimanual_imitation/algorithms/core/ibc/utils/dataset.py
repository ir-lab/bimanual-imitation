...
def load_tfrecord_dataset_sequence(
    ...
    def interleave_func(shard):
        dataset = tf.data.TFRecordDataset(shard, buffer_size=buffer_size_per_shard).cache().repeat()
    ...
    if for_rnn:
        return dataset.map(filter_episodes_rnn, num_parallel_calls=num_parallel_calls)
    else:
