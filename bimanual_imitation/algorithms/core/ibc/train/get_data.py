...
from bimanual_imitation.algorithms.core.ibc.utils import dataset as x100_dataset_tools
        ...
        if flatten_action:
            ...
            if eval_data:
                eval_data = eval_data.map(
                    lambda trajectory: trajectory._replace(action=flatten_action(trajectory.action))
                )

        # We predict 'many-to-one' observations -> action.
        train_data = train_data.map(
            lambda trajectory: ((trajectory.observation, trajectory.action[:, -1, Ellipsis]), ())
        )
        if eval_data:
            eval_data = eval_data.map(
                lambda trajectory: (
