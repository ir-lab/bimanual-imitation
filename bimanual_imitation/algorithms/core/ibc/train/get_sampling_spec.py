...
def get_sampling_spec(
    action_tensor_spec, min_actions, max_actions, uniform_boundary_buffer, act_norm_layer
):
    ...
    action_limit_nest = tf.nest.map_structure(
        generate_boundary_buffered_limits, action_tensor_spec, min_actions, max_actions
    )

    # Map up to the spec to avoid iterating over the tuples.
    buffered_min_actions = nest_utils.map_structure_up_to(
        action_tensor_spec, lambda a: a[0], action_limit_nest
    )
    buffered_max_actions = nest_utils.map_structure_up_to(
        action_tensor_spec, lambda a: a[1], action_limit_nest
    )
    ...
    def bounded_like(spec, min_action, max_action):
        return tensor_spec.BoundedTensorSpec(
            spec.shape, spec.dtype, minimum=min_action, maximum=max_action, name=spec.name
        )

    action_sampling_spec = tf.nest.map_structure(
        bounded_like, action_tensor_spec, normalized_min_actions, normalized_max_actions
    )
